...
class BorderedButton(tk.Button):
    def __init__(self, master, name=None, text="", command=None, font=None, width=20):
        self.border_frame = tk.Frame(master, highlightthickness=2, borderwidth=0)

        super().__init__(self.border_frame, name=name, text=text, command=command,  borderwidth=0, border=0, padx=0, pady=0, relief="flat", font=font, width=width)
        self.set_colors()
        super().pack()
